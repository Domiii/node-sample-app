...
module.exports = NoGapDef.component({
    ...
    Host: NoGapDef.defHost(function(SharedTools, Shared, SharedContext) {
        ...
        var tracePending = function(which, nPending) {
            if (!traceDepth) return;

            var frames = squishy.Stacktrace.getStacktrace();
            var iStartFrame = 1;
            var str = which + ' #' + nPending;
            if (!frames) {
                // internal call
                str += ' (internal)';
            }
                ...
                // call from user-code
                str += ' @';
                var n = Math.min(traceDepth + iStartFrame, frames.length);
                for (var i = iStartFrame; i < n; ++i) {
                    var frame = frames[i];
        ...
        return {
            ...
            _waitPendingAndSync: function(cb) {
            ...
            /**
             * When current set of requests have finished, call the given callback
             */
            waitFor: function(dbPromise) {
                ++this._nPending;

                tracePending('db operation start', this._nPending);
            ...
            initModels: function() {
                ...
                .then(function() {
                    ...
                    Shared.forEach(function(modelComponent) {
                        ...
                        modelComponentsByName[modelComponent._def.FullName] = modelComponent;
                        modelsByName[modelComponent._def.FullName] = modelComponent.Model;
            ...
            bulkCreate: function(model, objects) {
                ...
                for (var i = 0; i < objects.length; ++i) {
                    ...
                    chainer.add(model.create(object));
                };
                return chainer.run();
            },
